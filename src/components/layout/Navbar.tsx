              ...
              <div className="space-y-10 mb-12">
                ...
                {SERVICE_CATEGORIES.map((cat, idx) => (
                  <div key={idx} className="space-y-4">
                    <p className="text-[11px] font-black text-ecru-white/30 uppercase tracking-[0.2em]">{cat.title}</p>
...
/* --- LOGO & CTA REMAIN SAME AS PREVIOUS --- */
      ...
      {SERVICE_CATEGORIES.map((cat, idx) => (
        <div key={idx} className="space-y-10">
          <div className="flex items-center gap-5 border-b border-old-gold/20 pb-6">
            <span className="text-old-gold">{cat.icon === 'logo' ? <Image src={BRAND_DATA.logo} alt="" width={20} height={20} className="grayscale brightness-200" /> : <cat.icon size={20} />}</span>
            <h4 className="text-old-gold text-[11px] font-black tracking-[0.5em] uppercase">{cat.title}</h4>
          </div>
          <ul className="space-y-5">
            {services.filter(s => s.category === cat.title).map(service => (
              <li key={service.slug}>
                <Link href={`/services/${service.slug}`} className="text-ecru-white/50 hover:text-old-gold hover:translate-x-3 transition-all block font-black text-[18px] italic uppercase tracking-tighter leading-none">{service.title}</Link>
              </li>
            ))}
          </ul>
        </div>
